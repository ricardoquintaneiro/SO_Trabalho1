...
#Declaração de matriz e valores DEFAULT
export LANG=en_US.UTF-8 # Formatação do printf e da data
export LANGUAGE=en # Muda a linguagem da bash
declare -A matrix       # Matriz 2D com informações sobre os processos
column=6                # Valor DEFAULT da coluna de ordenação da matriz
reverse=""              # Valor DEFAULT da direção de ordenação
regex_positive_int='^[0-9]+$'   # Regex para verificação numérica
...
do
        ...
        # No fim, a informação que já temos (nome, data, user) é guardada num array ou num string, para termos menos trabalho no fim
...
for ((j=1;j<$i;j++)) do
        ...
        printf "%-20s \b%-9s \b%6s \b%11s \b%12s \b%12s \b%14s \b%14s\n" "${matrix[$j,1]}"\
         "${matrix[$j,2]}" "${matrix[$j,3]}" "${matrix[$j,4]}" "${matrix[$j,5]}" "${matrix[$j,6]}"\
          "${matrix[$j,7]}" "${matrix[$j,8]}"
done |
 sort -t $'\b' -"$reverse"g -k"$column" |
  ( ! [ -z "$p" ] && awk "NR<=$p" || awk "NR>0")

# Os valores finais de readb e writeb correspondem aos valores de leitura e escrita durante o tempo estipulado.